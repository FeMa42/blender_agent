...
class BlenderMCPServer:
    ...
    def import_model(self, file_path, name=None):
        ...
        try:
            ...
            if file_ext not in ['.glb', '.gltf', '.fbx', '.obj', '.stl', '.ply', '.blend']:
                return {"error": f"Unsupported file format: {file_ext}. Supported formats: .glb, .gltf, .fbx, .obj, .stl, .ply, .blend"}

            # Deselect all objects first (so we can identify new ones later)
            bpy.ops.object.select_all(action='DESELECT')
            ...
            # Ensure view layer is updated to reflect the imported objects
            bpy.context.view_layer.update()

            # Get all the newly imported objects (they should be selected)
            imported_objects = [
                obj for obj in bpy.context.view_layer.objects if obj.select_get()]

            # If no objects were imported/selected, try to find them by name
            if not imported_objects:
                # Get the basename without extension
                base_name = os.path.splitext(os.path.basename(file_path))[0]
                # Look for objects with names containing the base filename
                imported_objects = [obj for obj in bpy.context.scene.objects
                                    if base_name.lower() in obj.name.lower() and obj.type in ['MESH', 'ARMATURE', 'EMPTY']]

            if not imported_objects:
                return {"error": "No objects were imported or imported objects could not be identified"}

            # If name is provided, rename the parent or main object
            if name and imported_objects:
                # Find the parent or main object
                parent_objects = [
                    obj for obj in imported_objects if not obj.parent]

                if parent_objects:
                    main_obj = parent_objects[0]  # Take the first parent
                else:
                    # No parents, just use first object
                    main_obj = imported_objects[0]

                main_obj.name = name
                # If it has data (like mesh data), rename that too
                if hasattr(main_obj, 'data') and main_obj.data:
                    main_obj.data.name = name

                # Update the imported_objects list with the new name
                imported_objects = [obj if obj != main_obj else bpy.data.objects.get(
                    name) for obj in imported_objects]

            # Get information about the imported objects
            imported_names = [obj.name for obj in imported_objects]

            # Get the main object info
            main_obj = imported_objects[0]
            obj_info = {
                "name": main_obj.name,
                "type": main_obj.type,
                "location": [main_obj.location.x, main_obj.location.y, main_obj.location.z],
                "rotation": [main_obj.rotation_euler.x, main_obj.rotation_euler.y, main_obj.rotation_euler.z],
                "scale": [main_obj.scale.x, main_obj.scale.y, main_obj.scale.z],
            }

            if main_obj.type == "MESH":
                try:
                    bounding_box = self._get_aabb(main_obj)
                    obj_info["world_bounding_box"] = bounding_box
                except Exception as e:
                    # Handle the case where we can't get the bounding box
                    obj_info["world_bounding_box_error"] = str(e)

            # Return success with imported object info
            return {
                "success": True,
                "message": f"Successfully imported {file_path} with {len(imported_objects)} objects",
                "main_object": obj_info,
